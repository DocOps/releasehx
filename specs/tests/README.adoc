...
* JSON→AsciiDoc, HTML, Markdown conversion workflows
...
[source,bash]
----
rake rspec                   # RSpec tests only
rake test:cli               # CLI functionality
rake test:yaml              # YAML validation
rake test:mcp               # MCP resource smoke test
----
...
* Release data processing workflows (JSON→RHYML→output formats)
...
[source,bash]
----
 rake install:local  # Build and install locally
 rake test:pr        # Full test suite
----
...
*  JSON→Markdown conversion validation
*  JSON→AsciiDoc→PDF pipeline testing
